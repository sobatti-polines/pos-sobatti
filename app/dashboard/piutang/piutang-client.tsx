...
import { processBayarPiutang } from "./actions";
...
export default function PiutangClient({ 
  ...
  const [selectedPiutang, setSelectedPiutang] = useState<PiutangData | null>(null);
  ...
  const handleBayar = async () => {
    ...
    if (bayarAmount > selectedPiutang.sisa_piutang) {
      setError("Jumlah bayar melebihi sisa piutang");
      return;
    }
    ...
    const res = await processBayarPiutang({
      id_piutang: selectedPiutang.id,
        ...
        <div className="flex-1 overflow-y-auto min-h-0 relative">
          <Table>
            <TableHeader className="hidden md:table-header-group">
              <TableRow>
                <TableHead className="md:pl-6 text-left">Pelanggan</TableHead>
                <TableHead className="text-left">Tgl Piutang</TableHead>
            ...
            <TableBody>
              {data.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={8} className="text-center py-8 text-muted-foreground">Belum ada data piutang</TableCell>
                </TableRow>
              ) : (
                data.map(p => {
                  const pelangganName = Array.isArray(p.pelanggan) ? p.pelanggan[0]?.nama_pelanggan : p.pelanggan?.nama_pelanggan;
                  return (
                    <TableRow key={p.id} className="group hover:bg-muted/30 transition-colors flex flex-col md:table-row p-4 md:p-0 border-b">
                      <TableCell className="md:pl-6 py-2 md:py-4 block md:table-cell">
                        <span className="md:hidden text-xs font-medium text-muted-foreground uppercase tracking-widest mb-1 block">Pelanggan</span>
                        <span className="font-medium text-[15px] md:text-[14px]">{pelangganName || '-'}</span>
                      </TableCell>
                      <TableCell className="py-2 md:py-4 block md:table-cell tabular-nums">
                        <span className="md:hidden text-xs font-medium text-muted-foreground uppercase tracking-widest mb-1 block">Tgl Piutang</span>
                        {format(new Date(p.tanggal_piutang), 'dd MMM yyyy')}
                      </TableCell>
                      <TableCell className="py-2 md:py-4 block md:table-cell tabular-nums">
                        <span className="md:hidden text-xs font-medium text-muted-foreground uppercase tracking-widest mb-1 block">Jatuh Tempo</span>
                        {p.tanggal_jatuh_tempo ? format(new Date(p.tanggal_jatuh_tempo), 'dd MMM yyyy') : '-'}
                      </TableCell>
                      ...
                      <TableCell className="md:text-center py-2 md:py-4 block md:table-cell">
                        ...
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] uppercase font-semibold tracking-wider ${
                          p.status === 'lunas' ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'
                        }`}>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
          <div className="bg-background border border-border shadow-xl rounded-xl w-full max-w-md overflow-hidden flex flex-col max-h-full">
            <div className="px-6 py-4 border-b border-border flex items-center justify-between bg-muted/30">
              <h2 className="font-semibold text-lg">Bayar Piutang</h2>
              <button 
            ...
            <div className="p-6 overflow-y-auto space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4 bg-muted/30 p-4 rounded-lg">
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wider">Sisa Piutang</p>
                  <p className="font-semibold text-lg tabular-nums text-rose-600">{formatIDR(selectedPiutang.sisa_piutang)}</p>
                </div>
                <div>
                  <p className="text-xs text-muted-foreground uppercase tracking-wider">Jatuh Tempo</p>
                  <p className="font-medium">
                    {selectedPiutang.tanggal_jatuh_tempo ? format(new Date(selectedPiutang.tanggal_jatuh_tempo), 'dd MMM yyyy') : '-'}
                  </p>
                </div>
              </div>
            ...
            <div className="px-6 py-4 border-t border-border bg-muted/30 flex justify-end gap-3">
              <Button variant="ghost" onClick={() => setSelectedPiutang(null)} disabled={loading}>
                Batal
              </Button>
