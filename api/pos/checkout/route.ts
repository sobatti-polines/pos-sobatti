...
export async function POST(request: Request) {
  ...
  const { data: products } = await supabase
    .from("produk")
    .select("id, harga_modal, harga_jual_satuan, harga_jual_grosir")
    .in("id", productIds);
  ...
  for (const item of items) {
    ...
    const jumlah = harga_jual * item.qty;
    const profit = (harga_jual - prod.harga_modal) * item.qty;
  ...
  const { data: transaction, error: txError } = await supabase
    ...
    .insert({
      ...
      kembali,
      dp: 0,
      sisa: jumlah_bayar < total_tagihan ? total_tagihan - jumlah_bayar : 0,
    })
    .select("id")
    .single();
