...
-- Multi-unit selling: produk bisa dijual dalam satuan base (pcs/meter)
-- atau satuan besar (roll/lusin) dengan harga & rasio sendiri.

-- ============================================================
-- 1) Kolom baru di tabel produk
-- ============================================================
ALTER TABLE produk
  ADD COLUMN IF NOT EXISTS jual_satuan TEXT,
  ADD COLUMN IF NOT EXISTS jual_ratio NUMERIC NOT NULL DEFAULT 1,
  ADD COLUMN IF NOT EXISTS harga_jual_besar_satuan NUMERIC,
  ADD COLUMN IF NOT EXISTS harga_jual_besar_grosir  NUMERIC,
  ADD COLUMN IF NOT EXISTS harga_jual_besar_promo   NUMERIC;

COMMENT ON COLUMN produk.jual_satuan IS 'Nama satuan jual besar (contoh: ROLL). NULL = hanya base unit.';
COMMENT ON COLUMN produk.jual_ratio IS '1 satuan besar = berapa satuan base (contoh: 20 m/roll)';
COMMENT ON COLUMN produk.harga_jual_besar_satuan IS 'Harga jual per satuan besar — tier Satuan';
COMMENT ON COLUMN produk.harga_jual_besar_grosir  IS 'Harga jual per satuan besar — tier Grosir';
COMMENT ON COLUMN produk.harga_jual_besar_promo   IS 'Harga jual per satuan besar — tier Promo';

-- ============================================================
-- 2) Kolom baru di detail_transaksi_keluar (audit + display)
-- ============================================================
ALTER TABLE detail_transaksi_keluar
  ADD COLUMN IF NOT EXISTS satuan_jual TEXT,
  ADD COLUMN IF NOT EXISTS qty_satuan  INT,
  ADD COLUMN IF NOT EXISTS jual_ratio  NUMERIC;

COMMENT ON COLUMN detail_transaksi_keluar.satuan_jual IS 'Satuan yang dipilih saat menjual (METER/ROLL)';
COMMENT ON COLUMN detail_transaksi_keluar.qty_satuan  IS 'Jumlah dalam satuan jual (mis: 2 roll)';
COMMENT ON COLUMN detail_transaksi_keluar.jual_ratio  IS 'Snapshot rasio jual saat transaksi';

-- ============================================================
-- 3) Update process_checkout — multi-unit support
-- ============================================================
...
DECLARE
  ...
  v_qty                  numeric;          -- base qty (stock units)
  v_qty_satuan           numeric;          -- qty in sold unit
  v_satuan_jual          text;             -- sold unit name (or null for base)
  v_jual_ratio_snapshot  numeric;          -- snapshot of ratio
...
BEGIN
  ...
  LOOP
    ...
    -- Determine qty_satuan (qty in the sold unit) and base qty
    ...
    -- Compute base qty depending on sold unit
    IF v_satuan_jual IS NOT NULL AND v_prod.jual_satuan IS NOT NULL
       AND UPPER(v_satuan_jual) = UPPER(v_prod.jual_satuan)
    THEN
      -- Sold in big unit → base = qty × ratio
      v_jual_ratio_snapshot := COALESCE(v_prod.jual_ratio, 1);
      v_qty := v_qty_satuan * v_jual_ratio_snapshot;
    ELSE
      -- Sold in base unit
      v_jual_ratio_snapshot := 1;
      v_qty_satuan := v_qty;  -- ensure qty_satuan = base qty
      v_qty := (v_item->>'qty')::numeric;
      IF v_qty IS NULL OR v_qty <= 0 THEN
        v_qty := v_qty_satuan;
      END IF;
      v_qty_satuan := v_qty;
      v_satuan_jual := NULL;  -- NULL means base unit
    END IF;
    ...
    -- Price lookup: big unit or base unit?
    IF v_satuan_jual IS NOT NULL AND UPPER(v_satuan_jual) = UPPER(v_prod.jual_satuan) THEN
      -- Big unit pricing
      v_harga_jual := COALESCE(v_prod.harga_jual_besar_satuan, 0);
      IF v_type_harga = 'GROSIR' AND v_prod.harga_jual_besar_grosir IS NOT NULL THEN
        v_harga_jual := v_prod.harga_jual_besar_grosir;
      END IF;
      IF v_type_harga = 'PROMO' AND v_prod.harga_jual_besar_promo IS NOT NULL THEN
        v_harga_jual := v_prod.harga_jual_besar_promo;
      END IF;
    ELSE
      -- Base unit pricing
    ...
    v_details := v_details || jsonb_build_object(
      ...
      'qty',            v_qty,              -- base qty (stock units)
      'qty_satuan',     v_qty_satuan,       -- qty in sold unit
      'satuan_jual',    v_satuan_jual,      -- sold unit name (NULL = base)
  ...
  -- Insert detail rows (includes new columns: satuan_jual, qty_satuan, jual_ratio)
