-- Fix: 20260803_fix_checkout_stock_validation.sql
--
-- Perbaikan pada process_checkout():
-- 1. Validasi stok SEBELUM transaksi disimpan:
--    - Jika qty item <= 0            -> RAISE EXCEPTION 'Qty tidak valid'
--    - Jika qty > stok + stok_gudang -> RAISE EXCEPTION dengan nama produk
-- 2. Pengurangan stok dual-warehouse: kurangi stok display dulu, jika tidak
--    cukup, sisa dikurangi dari stok_gudang (tidak lagi menghasilkan stok negatif)
-- 3. Konsistensi riwayat_avco: stok_sebelum / stok_sesudah kini memakai
--    TOTAL stok (display + gudang), sama seperti process_barang_masuk,
--    agar get_inventory_value_at_date() dan Kartu Stok akurat.
...
BEGIN
  ...
  -- Validasi dilakukan sebelum insert agar transaksi batal total bila
  -- ada item yang stoknya tidak mencukupi.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    SELECT id, nama_produk, harga_modal, harga_jual_satuan, harga_jual_grosir, harga_jual_promo, hitung_stok, stok, stok_gudang, harga_pokok_avco, nilai_persediaan
    INTO v_prod
    FROM produk
    ...
    v_jumlah  := (v_harga_jual - v_diskon_item) * v_qty;
  ...
  -- Validate customer for credit/DP
  ...
  LOOP
    ...
    IF FOUND THEN
      ...
      -- Kurangi display stock dulu; sisanya dari gudang
      ...
      -- Record into riwayat_avco (stok = TOTAL stock, konsisten dgn barang_masuk)
