...
BEGIN
  ...
  LOOP
    ...
    v_profit  := (v_harga_jual - v_prod.harga_modal) * (v_item->>'qty')::numeric
                 - v_diskon_item * (v_item->>'qty')::numeric;
  ...
  INSERT INTO transaksi_keluar (
    ...
    v_total, p_bayar, v_kembali, v_dp, v_sisa
  )
  ...
  INSERT INTO detail_transaksi_keluar (
    ...
    diskon_item, qty, jumlah, kas_masuk, profit
  )
  SELECT
    v_tx_id,
    (d->>'id_produk')::integer,
  ...
  -- Deduct stock where hitung_stok = true
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    UPDATE produk
    SET stok = stok - (v_item->>'qty')::numeric
    WHERE id = (v_item->>'id_produk')::integer
      AND hitung_stok = true;
  END LOOP;
