...
-- T1-18: Guard `hitung_stok` di RPC process_barang_masuk.
--
-- Tujuan: mencegah request crafting menambah stok ke produk yang tidak
-- dihitung stoknya (`hitung_stok = false`, contoh: produk jasa/paket master).
-- Guard diterapkan SEKALI sebelum percabangan UoM/legacy, sehingga melindungi
-- kedua branch tanpa duplikasi.
--
-- Prinsip additive: CREATE OR REPLACE FUNCTION — logika inti (UoM, AVCO,
-- dual-stok, no_surat) TIDAK diubah. Aman dijalankan ulang.
--
-- Catatan verifikasi (dari T1-18): process_barang_masuk HANYA dipanggil oleh
-- server action addStockIn (stock-in). POS checkout memakai process_checkout,
-- jadi menambah guard di sini tidak berpengaruh ke penjualan.
