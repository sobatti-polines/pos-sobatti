...
-- Hapus produk.jual_ratio (Rasio Jual) karena redundant dengan conversion_ratio.
-- Satuan jual besar tetap ditentukan oleh produk.jual_satuan, tapi konversi
-- qty-nya memakai conversion_ratio (rasio yang sama dengan satuan BELI).
--
-- Latar belakang: barang masuk per lusin -> inventaris per pcs -> dijual hanya
-- pcs & lusin. Rasio 1 lusin = 12 pcs berlaku sama untuk BELI dan JUAL.
--
-- detail_transaksi_keluar.jual_ratio TETAP ada sebagai snapshot transaksi
-- (untuk invoice & audit transaksi lama).

-- ============================================================
-- 1) Backfill: pindahkan nilai jual_ratio yang bermakna ke conversion_ratio
-- ============================================================
UPDATE produk
SET conversion_ratio = GREATEST(COALESCE(conversion_ratio, 1), COALESCE(jual_ratio, 1))
WHERE jual_ratio IS NOT NULL;

-- ============================================================
-- 2) Drop kolom jual_ratio dari tabel produk
-- ============================================================
ALTER TABLE produk
  DROP COLUMN IF EXISTS jual_ratio;

-- ============================================================
-- 3) Update process_checkout — konversi jual besar pakai conversion_ratio
--    (matching satuan jual tetap memakai jual_satuan)
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    THEN
      -- Sold in big unit → base = qty × conversion_ratio
  ...
  -- Insert detail rows (snapshot kolom satuan_jual, qty_satuan, jual_ratio)
