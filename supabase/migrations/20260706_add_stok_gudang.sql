-- Add warehouse stock column to produk
-- stok becomes display stock, stok_gudang = warehouse stock
ALTER TABLE produk ADD COLUMN stok_gudang numeric DEFAULT 0;

-- Update process_checkout to use total stock for nilai_persediaan calculation
...
BEGIN
  ...
  LOOP
    SELECT id, harga_modal, harga_jual_satuan, harga_jual_grosir, harga_jual_promo, hitung_stok, stok, stok_gudang, harga_pokok_avco, nilai_persediaan
    INTO v_prod
    FROM produk
  ...
  -- Validate customer for credit/receivable
  ...
  -- Create Piutang if there is a balance
  IF v_sisa > 0 THEN
    INSERT INTO piutang_dagang (
      id_transaksi_keluar, id_pelanggan, tanggal_piutang, jumlah_awal, status
    ) VALUES (
      v_tx_id, p_id_pelanggan, now(), v_sisa, 'belum_lunas'
    );
  END IF;
  ...
  -- Deduct display stock where hitung_stok = true and record AVCO mutation
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
  LOOP
    ...
    IF FOUND THEN
      v_stok_total := COALESCE(v_prod.stok, 0) + COALESCE(v_prod.stok_gudang, 0);
      ...
      INSERT INTO riwayat_avco (
        ...
        v_prod.harga_pokok_avco,
        (v_stok_total - (v_item->>'qty')::numeric) * v_prod.harga_pokok_avco
      );

      -- Update product display stock
      UPDATE produk
      SET stok = stok - (v_item->>'qty')::numeric,
          nilai_persediaan = (COALESCE(stok, 0) - (v_item->>'qty')::numeric + COALESCE(stok_gudang, 0)) * harga_pokok_avco
      WHERE id = (v_item->>'id_produk')::integer;
    END IF;
  END LOOP;
