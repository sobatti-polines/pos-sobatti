-- 20260824_admin_stockin_no_price.sql
-- Modifikasi process_barang_masuk: jika total_cost = 0, stok naik tapi AVCO TIDAK dihitung.
-- 
-- Konteks: Owner ingin admin hanya input jumlah stok (tanpa harga).
-- Harga beli akan ditentukan owner kemudian via fitur "Tentukan Harga".
--
-- Perubahan:
-- - UoM branch: jika v_total_cost = 0 → skip AVCO, stok_gudang saja
-- - Legacy branch: jika harga_beli = 0 → skip AVCO, stok_gudang saja
-- - Riwayat AVCO tetap dicatat (dengan harga_satuan_transaksi = 0)
-- - UPDATE produk: jika total_cost = 0 → tidak ubah harga_pokok_avco/nilai_persediaan
...
DECLARE
  ...
  v_has_cost         BOOLEAN;  -- NEW: flag apakah ada harga
...
BEGIN
  ...
  LOOP
    ...
    IF v_is_uom THEN
      ...
      -- NEW: Tentukan apakah ada harga yang valid
      ...
      -- NEW: Hanya hitung AVCO jika ada harga
      IF v_has_cost THEN
      ...
      ELSE
        -- Tanpa harga: pertahankan AVCO & nilai persediaan yang lama
    ...
    SET
      ...
      -- NEW: Hanya update AVCO & nilai_persediaan jika ada harga
    ...
    -- NEW: Tandai harga_ditentukan di barang_masuk
    UPDATE barang_masuk
