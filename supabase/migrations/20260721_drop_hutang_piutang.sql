-- Reverse: Remove all hutang/piutang tables and references
-- Old migration files are kept as history reference

-- 1. Update process_checkout to remove piutang insertion
...
BEGIN
  ...
  -- Piutang insertion removed — feature eliminated
...
-- 2. Drop hutang/piutang tables (CASCADE will drop their RLS policies too)
...
-- 3. Update sequence for transaksi numbering (remove hutang/piutang interference)
-- No sequence changes needed, the piutang referencing columns remain in transaksi_keluar
-- (dp, sisa columns stay for transaction completeness tracking, but no linked piutang entry)
