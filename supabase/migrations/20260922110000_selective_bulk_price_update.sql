DROP FUNCTION IF EXISTS public.bulk_adjust_product_prices(
...
CREATE OR REPLACE FUNCTION public.bulk_adjust_product_prices(
  ...
  p_selected_product_ids integer[] DEFAULT NULL
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_apply AND coalesce(cardinality(p_selected_product_ids), 0) = 0 THEN
    RAISE EXCEPTION 'Pilih minimal 1 produk';
  END IF;
  ...
  IF p_apply THEN
    ...
    WHERE p.id = c.id
      AND p.id = ANY (p_selected_product_ids);
...
REVOKE ALL ON FUNCTION public.bulk_adjust_product_prices(
  integer, text, text, numeric, numeric, boolean, boolean, boolean, boolean, boolean, boolean, boolean, integer[]
) FROM PUBLIC;
